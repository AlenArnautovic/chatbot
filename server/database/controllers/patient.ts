...
//Ansonsten NULL
export const checkPatientsData = async (age:number ,prename:string, lastname:string, vNumber:string, req:Request, res:Response) => {        
    console.log(NAMESPACE, "Check Patient Data");
    const query = 'SELECT "Yes" AS Correct_PATIENT_Data FROM 	CHATBOT.PATIENT WHERE floor(timestampdiff(day,birthdate,sysdate())/365) = ? AND		PRENAME = ? AND		LASTNAME = ? AND		vnumber = ?'; 
       
    (await Connection).query(query, [age, prename, lastname, vNumber], (error, result) => {
        if (error) {
...
export const checkIfDoctorForDiseaseIsAvailable = async (disease:string, req:Request, res:Response) => {        
    console.log(NAMESPACE, "Retrieve first 5 avaialable Appointments from Doc to a Speficic Disease");
...
export const checkIfDoctorForDiseaseIsAvailableForASpecificAppointment = async (disease:string,appointment:string, req:Request, res:Response) => {        
    console.log(NAMESPACE, "Retrieve first 5 avaialable Appointments from Doc to a Speficic Disease and Appointment");
...
export const addPatient = async (vNumber:string, preName:string,lastName:string, phone:string, birthdate:string, address_ID:string, req:Request, res:Response) => {        
    ...
    const query = 'INSERT INTO Chatbot.Patient(vNumber,PreName,LastName,Phone,Birthdate,Address_ID) VALUES (?,?,?,?,?,?);';
    (await Connection).query(query, [vNumber, preName,lastName, phone, birthdate, address_ID], (error, result) => {
        if (error) {
            console.log(NAMESPACE, error.message, error);    
            return res.status(500).json({message:error.message,error});
        }
...
//Delete appointment
export const deleteAppointment = async (vNumber:string, appointment:string, req:Request, res:Response) => {        
    console.log(NAMESPACE, "Delete Appointment" );
    const query = 'DELETE FROM chatbot.APPOINTMENT WHERE pat_id = (SELECT patient_id FROM chatbot.PATIENT WHERE vnumber =?) AND openinghours_id = (SELECT openinghours_id FROM chatbot.OPENINGHOURS WHERE time_from = ?);';
    (await Connection).query(query, [vNumber, appointment], (error, result) => {
        if (error) {
            console.log(NAMESPACE, error.message, error);    
            return res.status(500).json({message:error.message,error});
